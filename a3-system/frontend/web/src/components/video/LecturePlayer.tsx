      ...
      return (
        <span
          ...
          className={`font-bold transition-all duration-500 ${
            isActive
              ? "text-[#4a5568] underline decoration-[#8a9ba3] decoration-2"
              : "text-[#555]"
          }`}
        >
          {part}
...
export default function LecturePlayer({ data, topic }: Props) {
  ...
  return (
    <div ref={containerRef} className="flex flex-col h-full select-none">
      ...
      ════════════════════════════════════════ */}
      <div className="h-[55%] min-h-0 relative overflow-hidden rounded-xl border border-[#D6CFC2]">
        {/* Subtle grid background */}
        <div
          className="absolute inset-0 transition-transform duration-[3000ms] ease-in-out"
          style={{ transform: kenBurnsTransform }}
        >
          <div className="absolute inset-0 bg-gradient-to-br from-[#F7F5F0] to-[#E7E2D7]" />
          <div
            className="absolute inset-0 opacity-[0.15]"
            style={{
              backgroundImage:
                "linear-gradient(rgba(184,195,201,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(184,195,201,0.4) 1px, transparent 1px)",
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at center, rgba(184,195,201,0.15) 0%, transparent 70%)",
            }}
          />
        </div>
        ...
        >
          ...
          <div className="absolute top-4 left-4 flex items-center gap-2">
            <span className="text-[9px] font-mono text-[#888] bg-white/60 px-2 py-0.5 rounded-md border border-[#D6CFC2]">
              {slide.number} / {slides.length}
            </span>
            {slide.audio_url && ttsProvider !== "browser" && (
              <span className="text-[9px] text-green-600 flex items-center gap-1 bg-white/60 px-2 py-0.5 rounded-md border border-[#D6CFC2]">
                <Volume2 className="w-2.5 h-2.5" />
                Audio ready
              </span>
            )}
            {!slide.audio_url && ttsProvider !== "browser" && (
              <span className="text-[9px] text-amber-600 flex items-center gap-1 bg-white/60 px-2 py-0.5 rounded-md border border-[#D6CFC2]">
          ...
          {/* Header */}
          <h2 className="text-lg md:text-xl font-black text-[#2a2a2a] text-center mb-4 leading-tight tracking-tight">
          ...
          <div className="space-y-2 max-w-md w-full">
            {slide.bullets?.map((bullet, i) => (
              ...
              >
                <div className="w-1.5 h-1.5 rounded-full bg-[#8a9ba3] mt-2 shrink-0" />
                <span className="text-sm text-[#444] leading-relaxed font-medium">
          ...
          {slide.visual_hint && (
            <div className="mt-4 px-3 py-2 rounded-lg bg-[#B8C3C9]/10 border border-[#B8C3C9]/30 text-[10px] text-[#666] italic">
              <span className="font-semibold text-[#8a9ba3]">Visual: </span>
              {slide.visual_hint}
            </div>
          )}
        ...
        {/* Slide progress bar */}
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-[#D6CFC2]">
          <div
            className="h-full bg-gradient-to-r from-[#B8C3C9] to-[#8a9ba3] transition-all duration-100 ease-linear"
      ...
      <div className="flex-1 min-h-0 flex flex-col mt-2">
        {/* Transcript header */}
        <div className="flex items-center justify-between px-3 py-2 bg-[#E7E2D7]/50 rounded-t-xl border border-[#D6CFC2] border-b-0">
          <div className="flex items-center gap-2">
            <ScrollText className="w-3.5 h-3.5 text-[#8a9ba3]" />
        ...
        {/* Transcript content */}
        <div
          className="flex-1 overflow-y-auto bg-white rounded-b-xl border border-[#D6CFC2] border-t-0 p-3 space-y-2 scrollbar-hide"
          ...
          {slides.map((s, idx) => (
            <button
              ...
              className={`w-full text-left p-2.5 rounded-lg border transition-all duration-200 ${
                idx === currentSlide
                  ? "bg-[#B8C3C9]/10 border-[#B8C3C9]/40 shadow-sm"
                  : idx < currentSlide
                    ? "bg-[#F7F5F0]/50 border-transparent hover:bg-[#E7E2D7]/30"
                    : "bg-white border-transparent opacity-60 hover:opacity-80 hover:bg-[#F7F5F0]/50"
              }`}
            >
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                    idx === currentSlide
                      ? "bg-[#B8C3C9] text-white"
                      : "bg-[#D6CFC2] text-[#888]"
                  }`}
                >
                  {s.number}
                </span>
                <span className="text-[10px] font-semibold text-[#4a5568] truncate">
                  {s.header}
                </span>
                {idx === currentSlide && isPlaying && (
                  <span className="ml-auto flex items-center gap-1">
                    <span className="w-1 h-1 rounded-full bg-[#8a9ba3] animate-pulse" />
                    <span className="text-[9px] text-[#8a9ba3]">Playing</span>
                  </span>
                )}
              </div>
      ...
      <div className="mt-2 space-y-2 shrink-0">
            ...
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                i === currentSlide
                  ...
                  : i < currentSlide
                    ? "w-1.5 bg-[#B8C3C9]"
                    : "w-1.5 bg-[#D6CFC2] hover:bg-[#B8C3C9]"
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex items-center justify-between px-1">
          <div className="flex items-center gap-1">
            <button
              onClick={restart}
              className="p-1.5 rounded-lg text-[#888] hover:text-[#555] hover:bg-[#E7E2D7] transition-all"
              title="Restart"
            >
            ...
            <button
              ...
              className="p-1.5 rounded-lg text-[#888] hover:text-[#555] hover:bg-[#E7E2D7] transition-all disabled:opacity-20 disabled:cursor-not-allowed"
            >
          ...
          <button
            ...
            className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
              isPlaying
                ? "bg-[#B8C3C9]/30 text-[#4a5568] shadow-md"
                : "bg-[#E7E2D7] text-[#666] hover:bg-[#B8C3C9]/30 hover:text-[#4a5568]"
            }`}
          >
        ...
        {/* Info bar */}
        <div className="flex items-center justify-between px-2 text-[9px] text-[#999]">
