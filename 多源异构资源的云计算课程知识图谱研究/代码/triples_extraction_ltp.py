...
class LtpParser:
    def __init__(self):
        self.ltp = LTP()
        self.ltp.init_dict(path="filter/newwords.txt", max_window=8)
...
def run():
    ...
    with open("filter/brevity.txt", "r", encoding="UTF-8") as f:
        texts = []
        for line in f:
    ...
    with open('filter/equal.csv', 'r', encoding="UTF-8") as f:
        ...
        for w in equal:
            for i in w[1:]:
                if i != '':
                    should_equal.append(i)

    with open("result.csv", "w", newline='', encoding="UTF-8") as f, open("result_to_neo4j.csv", "w", newline='',
